...
public final class ReportsCommand implements BasicCommand {
    ...
    @Override public void execute(@NotNull CommandSourceStack source,@NotNull String[] args){CommandSender sender=source.getSender();if(!sender.hasPermission("mreports.staff")){sender.sendMessage(ReportService.color("&cНедостаточно прав."));return;}
        if(args.length==0){if(sender instanceof Player player)gui.open(player);else list(sender);return;}
        if(args[0].equalsIgnoreCase("reload")){if(!sender.hasPermission("mreports.reload")){sender.sendMessage(ReportService.color("&cНедостаточно прав."));return;}try{plugin.reloadRuntime();sender.sendMessage(ReportService.color("&aКонфигурация перезагружена."));}catch(RuntimeException e){sender.sendMessage(ReportService.color("&cОшибка: "+e.getMessage()));}return;}
        ...
        switch(args[0].toLowerCase(Locale.ROOT)){
            case "claim"->service.repository().claim(id,actor,sender.getName()).thenAccept(ok->reply(sender,ok,"Репорт взят."));
            case "resolve","reject"->{ReportStatus status=args[0].equalsIgnoreCase("resolve")?ReportStatus.RESOLVED:ReportStatus.REJECTED;String note=args.length<3?"":String.join(" ",java.util.Arrays.copyOfRange(args,2,args.length));service.repository().close(id,actor,sender.getName(),status,note).thenAccept(ok->reply(sender,ok,"Репорт закрыт: "+status));}
            case "note"->{if(args.length<3){usage(sender);return;}String note=String.join(" ",java.util.Arrays.copyOfRange(args,2,args.length));service.repository().addNote(id,actor,sender.getName(),note).thenAccept(ok->reply(sender,ok,"Заметка добавлена."));}
            case "view"->service.repository().find(id).thenAccept(found->plugin.scheduler().global(()->sender.sendMessage(found.map(r->ReportService.color("&e#"+r.id()+" &f"+r.reporterName()+" -> "+r.targetName()+" [&e"+r.category()+"&f] "+r.status()+" x"+r.duplicateCount()+"\n&7"+r.details())).orElse(ReportService.color("&cРепорт не найден.")))));
            default->usage(sender);
        }}
    private void list(CommandSender sender){service.repository().queue(20).thenAccept(rows->plugin.scheduler().global(()->{sender.sendMessage(ReportService.color("&eОткрытые репорты: "+rows.size()));rows.forEach(r->sender.sendMessage(ReportService.color("&7#"+r.id()+" &f"+r.targetName()+" &e"+r.category()+" &7"+r.status())));}));}
    private void reply(CommandSender sender,boolean ok,String success){plugin.scheduler().global(()->sender.sendMessage(ReportService.color(ok?"&a"+success:"&cОперация отклонена: проверьте ID, статус и обработчика.")));}
